...
function Note:fixRandom()
	local lastTime = self.last and self.last.time or self.time
	local nextTime = self.next and self.next.time or self.time
	self.time = math.max(lastTime+_hitRange, math.min(self.time, nextTime-_hitRange))
end
...
function Note:__render(camera)
	...
	local dir, time, target, speed, psx, psy, psz, prx, pry, prz = self.direction, self.displayTime, self._targetTime, self.speed,
